    ...
    public function index(Request $request): Response
    {
        ...
        $washerStats = DB::table('tickets')
            ...
            ->select(
                ...
                DB::raw("SUM(CASE WHEN tickets.status = 'paid' THEN tickets.total_cents ELSE 0 END) as revenue_cents"),                // Average duration in seconds between started_at and completed_at
        ...
        $result = $washers->map(function ($washer) use ($washerStats) {
            $stats = $washerStats->get($washer->id);

            $ticketsCompleted   = (int)  ($stats?->tickets_completed ?? 0);
            $ticketsTotal       = (int)  ($stats?->tickets_total ?? 0);
            $revenueCents       = (int)  ($stats?->revenue_cents ?? 0);
            $avgDurationSeconds = (float)($stats?->avg_duration_seconds ?? 0);
            $avgDurationMinutes = $avgDurationSeconds > 0 ? round($avgDurationSeconds / 60, 1) : null;

            // Tickets per hour: based on total active time span (first start → last complete)
            $ticketsPerHour = null;
            if ($stats?->first_started && $stats?->last_completed && $ticketsCompleted > 0) {
